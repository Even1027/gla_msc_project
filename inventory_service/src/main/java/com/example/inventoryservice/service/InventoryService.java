...
public class InventoryService {
    ...
    public void handleOrderEvent(
        ...
        // 添加明显的日志输出
        System.out.println("===============================");
        System.out.println("收到Kafka消息: " + orderEventJson);
        System.out.println("===============================");

        logger.info("接收到订单事件: partition={}, offset={}, json={}",
                partition, offset, orderEventJson);

        try {
            // 手动解析JSON字符串
            OrderEvent orderEvent = parseOrderEvent(orderEventJson);
            ...
            logger.error("订单事件处理失败: json={}, error={}",
                    orderEventJson, e.getMessage(), e);
        }
    }
    ...
    private OrderEvent parseOrderEvent(String json) {
        try {
            ...
            orderEvent.setProductId(jsonNode.get("productId").asLong());
            orderEvent.setQuantity(jsonNode.get("quantity").asInt());
            orderEvent.setStatus(jsonNode.get("status").asText());

            return orderEvent;
        } catch (Exception e) {
            logger.error("JSON解析失败: {}", json, e);
            throw new RuntimeException("无法解析订单事件JSON", e);
        }
    }
